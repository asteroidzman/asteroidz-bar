...
enum background_mode parse_background_mode(const char *mode) {
	...
	asteroidzbg_log(LOG_ERROR, "Unsupported background mode: %s", mode);
...
cairo_surface_t *load_background_image(const char *path) {
	...
	if (!pixbuf) {
		asteroidzbg_log(LOG_ERROR, "Failed to load background image (%s).",
				err->message);
		return NULL;
	}
	...
	if (!image) {
		asteroidzbg_log(LOG_ERROR, "Failed to read background image.");
		return NULL;
	}
	if (cairo_surface_status(image) != CAIRO_STATUS_SUCCESS) {
		asteroidzbg_log(LOG_ERROR, "Failed to read background image: %s."
#if !HAVE_GDK_PIXBUF
