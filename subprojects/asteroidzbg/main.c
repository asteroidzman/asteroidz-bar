...
struct asteroidzbg_state {
	...
	struct cm_state cm;
	struct wl_list configs;  // struct asteroidzbg_output_config::link
	struct wl_list outputs;  // struct asteroidzbg_output::link
	struct wl_list images;   // struct asteroidzbg_image::link
	bool run_display;
};

struct asteroidzbg_image {
	struct wl_list link;
	const char *path;
	bool load_required;
};

struct asteroidzbg_output_config {
	char *output;
	const char *image_path;
	struct asteroidzbg_image *image;
	enum background_mode mode;
	uint32_t color;
	struct wl_list link;
};

struct asteroidzbg_output {
	uint32_t wl_name;
	struct wl_output *wl_output;
	char *name;
	char *identifier;

	struct asteroidzbg_state *state;
	struct asteroidzbg_output_config *config;
...
/* Loads path as HDR if it carries HDR colorimetry, otherwise through the
 * ordinary SDR path. Always returns an hdr_image (with is_hdr false for the
 * SDR case) so callers have a single type to reason about. */
static struct hdr_image *load_wallpaper_image(const char *path) {
	struct hdr_image *img = hdr_image_load(path);
	if (img) {
		return img;
	}
...
static struct wl_buffer *draw_buffer(const struct asteroidzbg_output *output,
	...
	/* A 10-bit buffer is only worth requesting when the image really is HDR
	 * and the compositor can be told what its pixels mean -- an untagged
	 * PQ buffer would be read as plain gamma and look badly washed out. */
	bool use_hdr = image && image->is_hdr &&
		cm_can_represent(&output->state->cm, image);
	...
	if (image && image->surface) {
		render_background_image(cairo, image->surface,
...
static void get_buffer_size(const struct asteroidzbg_output *output,
...
static void render_frame(struct asteroidzbg_output *output,
	...
	if (buffer_width != output->buffer_width ||
		...
		/* Tag before the commit below: the image description is latched by
		 * the same commit that attaches the buffer it describes, so the
		 * compositor never sees PQ pixels it has not been told about. */
		cm_apply_to_surface(&output->state->cm, output->surface, image);
...
static void destroy_asteroidzbg_image(struct asteroidzbg_image *image) {
	if (!image) {
		return;
	}
...
static void destroy_asteroidzbg_output_config(struct asteroidzbg_output_config *config) {
	if (!config) {
		return;
	}
...
static void destroy_asteroidzbg_output(struct asteroidzbg_output *output) {
	if (!output) {
		return;
	}
...
static void layer_surface_configure(void *data,
	...
	struct asteroidzbg_output *output = data;
	output->width = width;
...
static void layer_surface_closed(void *data,
		struct zwlr_layer_surface_v1 *surface) {
	struct asteroidzbg_output *output = data;
	asteroidzbg_log(LOG_DEBUG, "Destroying output %s (%s)",
			output->name, output->identifier);
	destroy_asteroidzbg_output(output);
}
...
static void create_layer_surface(struct asteroidzbg_output *output) {
...
static void output_done(void *data, struct wl_output *wl_output) {
	struct asteroidzbg_output *output = data;
	if (!output->config) {
		asteroidzbg_log(LOG_DEBUG, "Could not find config for output %s (%s)",
				output->name, output->identifier);
		destroy_asteroidzbg_output(output);
	} else if (!output->layer_surface) {
		asteroidzbg_log(LOG_DEBUG, "Found config %s for output %s (%s)",
...
static void find_config(struct asteroidzbg_output *output, const char *name) {
	struct asteroidzbg_output_config *config = NULL;
...
static void output_description(void *data, struct wl_output *wl_output,
	...
	if (paren) {
		...
		if (!output->identifier) {
			asteroidzbg_log(LOG_ERROR, "Failed to allocate output identifier");
			return;
		}
...
static void handle_global(void *data, struct wl_registry *registry,
	...
	struct asteroidzbg_state *state = data;
	if (strcmp(interface, wl_compositor_interface.name) == 0) {
		...
		struct asteroidzbg_output *output = calloc(1, sizeof(struct asteroidzbg_output));
		output->state = state;
		output->scale = 1;
		output->wl_name = name;
		output->wl_output =
	...
	} else if (cm_state_handle_global(&state->cm, registry, name, interface,
			version)) {
...
static void handle_global_remove(void *data, struct wl_registry *registry,
		uint32_t name) {
	struct asteroidzbg_state *state = data;
	struct asteroidzbg_output *output, *tmp;
...
static bool store_asteroidzbg_output_config(struct asteroidzbg_state *state,
		struct asteroidzbg_output_config *config) {
	struct asteroidzbg_output_config *oc = NULL;
...
static void parse_command_line(int argc, char **argv,
	...
	struct asteroidzbg_output_config *config = calloc(1, sizeof(struct asteroidzbg_output_config));
	...
	while (1) {
		...
		case 'c':  // color
			if (!parse_color(optarg, &config->color)) {
				asteroidzbg_log(LOG_ERROR, "%s is not a valid color for asteroidzbg. "
		...
		case 'm':  // mode
			...
			if (config->mode == BACKGROUND_MODE_INVALID) {
				asteroidzbg_log(LOG_ERROR, "Invalid mode: %s", optarg);
			}
			break;
		case 'o':  // output
			if (config && !store_asteroidzbg_output_config(state, config)) {
				// Empty config or merged on top of an existing one
				destroy_asteroidzbg_output_config(config);
			}
			config = calloc(1, sizeof(struct asteroidzbg_output_config));
			config->output = strdup(optarg);
			config->mode = BACKGROUND_MODE_INVALID;
			wl_list_init(&config->link);  // init for safe removal
			break;
		case 'v':  // version
			fprintf(stdout, "asteroidzbg version " ASTEROIDZBG_VERSION "\n");
			exit(EXIT_SUCCESS);
			break;
		default:
	...
	if (optind < argc) {
		config = NULL;
		struct asteroidzbg_output_config *tmp = NULL;
...
int main(int argc, char **argv) {
	asteroidzbg_log_init(LOG_DEBUG);

	struct asteroidzbg_state state = {0};
	...
	wl_list_for_each(config, &state.configs, link) {
		...
		image = calloc(1, sizeof(struct asteroidzbg_image));
	...
	if (!state.display) {
		asteroidzbg_log(LOG_ERROR, "Unable to connect to the compositor. "
	...
	if (wl_display_roundtrip(state.display) < 0) {
		asteroidzbg_log(LOG_ERROR, "wl_display_roundtrip failed");
		return 1;
	}
	if (state.compositor == NULL || state.shm == NULL ||
			state.layer_shell == NULL) {
		asteroidzbg_log(LOG_ERROR, "Missing a required Wayland interface");
		return 1;
	}
	...
	while (wl_display_dispatch(state.display) != -1 && state.run_display) {
		...
		struct asteroidzbg_output *output;
		...
		wl_list_for_each(image, &state.images, link) {
			...
			struct hdr_image *loaded = load_wallpaper_image(image->path);
			if (!loaded) {
				asteroidzbg_log(LOG_ERROR, "Failed to load image: %s", image->path);
				continue;
			}
			...
			wl_list_for_each(output, &state.outputs, link) {
				if (output->dirty && output->config->image == image) {
					output->dirty = false;
					render_frame(output, loaded);
				}
			}

			image->load_required = false;
			hdr_image_destroy(loaded);
		}
	...
	struct asteroidzbg_output *output, *tmp_output;
	wl_list_for_each_safe(output, tmp_output, &state.outputs, link) {
		destroy_asteroidzbg_output(output);
	}

	struct asteroidzbg_output_config *tmp_config = NULL;
	wl_list_for_each_safe(config, tmp_config, &state.configs, link) {
		destroy_asteroidzbg_output_config(config);
	}

	struct asteroidzbg_image *tmp_image;
	wl_list_for_each_safe(image, tmp_image, &state.images, link) {
		destroy_asteroidzbg_image(image);
	}
