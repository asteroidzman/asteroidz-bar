...
# Find the pill rather than assume where it is.
#
# The other panels' tests hardcode an offset from the right edge, which holds
# for an icon-only pill and for the clock. The network module draws its own
# artwork at its own width, and the hardcoded X missed it completely -- the
# test then reported "the panel did not open" when the truth was "the click
# landed on the wallpaper".
PILL_X="$(python3 - "$WORK/idle.png" "$PILL_Y" <<'FINDPILL'
import sys
from PIL import Image
im = Image.open(sys.argv[1]).convert("RGB")
px = im.load()
w, h = im.size
y = int(sys.argv[2])
xs = [x for x in range(w) if sum(px[x, y]) < 400]   # panel is darker than the wallpaper
print((min(xs) + max(xs)) // 2 if xs else w // 2)
FINDPILL
)"
echo "  (pill centre found at x=$PILL_X)"
...
if [ "$OPENED" -gt $((IDLE + 400)) ]; then
	ok "clicking the network pill opens top by throughput ($IDLE -> $OPENED px)"
else
	bad "clicking the network pill opens top by throughput ($IDLE -> $OPENED px)"
fi
