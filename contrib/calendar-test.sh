#!/usr/bin/env bash
# calendar-test.sh — the clock's calendar panel.
#
...
if grep -q "Type .* unavailable\|Failed to load configuration" "$WORK/qs.log" 2>/dev/null; then
	bad "the shell loads with the calendar in it"
	grep -E "ERROR|unavailable" "$WORK/qs.log" | head -5
else
	ok "the shell loads with the calendar in it"
fi
...
if [ "$OPENED" -gt $((IDLE + 400)) ]; then
	ok "clicking the clock opens the calendar ($IDLE -> $OPENED px)"
else
	bad "clicking the clock opens the calendar ($IDLE -> $OPENED px)"
fi
...
if [ "$CLOSED" -lt $((OPENED - 400)) ]; then
	ok "clicking it again closes the calendar ($OPENED -> $CLOSED px)"
else
